...
int main(int argc, char **argv)
{
	...
	{
		if (!rank)
			cout << "Параметры командной строки: " << argv[0] << " N filename" << endl;
		MPI_Finalize();
		return 0;
	}

	int n = atoi(argv[1]);
	unsigned long long m = 1 << n, k, i;
	k = m / size;

	MPI_File file;
	if (MPI_File_open(comm, argv[2], MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &file))
	{
		if (!rank)
